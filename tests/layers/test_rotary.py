...
def test_rotary_2d():
    ...
    query = torch.empty(2, len(patch_positions), num_heads, head_dim, dtype=torch.float32, device=device).normal_()
    ...
    pixtral_rotary = transformers.models.pixtral.modeling_pixtral.PixtralRotaryEmbedding(pixtral_config).to(device)
    ...
    kwargs = {VisionKwargs.patch_positions: patch_positions, AttentionKwargs.device: device}
