...
def test_gpt_blended():
    ...
    sampled = BlendedDataset(
        "blended",
        [
            GPTMemmapDataset("memmap", DATASET_PREFIX).sample(get_sampling_config(6, sequence_length=5)),
            GPTMemmapDataset("memmap", DATASET_PREFIX).sample(get_sampling_config(2, sequence_length=5)),
        ],
        [0.75, 0.25],
