...
def test_gpt_blended_legacy_data():
    ...
    print(np.stack(samples[PhaseType.training]).tolist())
    Assert.all_equal(
        np.stack(samples[PhaseType.training]),
        np.array(GPT_BLENDED_EXPECTED_SAMPLES),
    )
...
GPT_BLENDED_MIXED_EXPECTED_SAMPLES = [
    [909, 6042, 8076, 87, 3410, 356],
    [488, 315, 1378, 1070, 2462, 4586],
    [370, 207, 279, 2094, 480, 2867],
    [533, 67, 207, 583, 1850, 84],
    [1378, 66, 243, 481, 3378, 86],
    [7091, 1850, 73, 727, 909, 77],
    [1014, 279, 73, 5477, 89, 207],
    [3149, 89, 4234, 451, 4066, 3149],
]
