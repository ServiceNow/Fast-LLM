...
def test_write_memmap_dataset(dtype):
    documents = [
        GPTSample(torch.from_numpy(np.random.randint(1000, size=np.random.randint(1, 100)).astype(dtype)))
        for _ in range(100)
    ]
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i, document in enumerate(documents):
            assert np.array_equal(
                dataset.get_document(i).token_ids, document.token_ids, equal_nan=True
            ), f"Mismatch for document {i}: {document} != {dataset.get_document(i)}."
...
def test_write_memmap_preference_dataset(dtype):
    def generate_valid_span(max_seq_length):
        ...
        return torch.from_numpy(np.sort(span))
    ...
    documents = [
        GPTSample(
            token_ids=torch.from_numpy(np.random.randint(vocab_size, size=max_seq_length).astype(dtype)),
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        dataset = GPTMemmapDataset(name="foo", prefix=prefix)
        for i, document in enumerate(documents):
            dataset_item = dataset.get_document(
                i,
            ...
            ), f"Token ids mismatch for document {i}: {document} != {dataset.get_document(i)}."

            assert np.array_equal(
                dataset_item.chosen_span, document.chosen_span, equal_nan=True
            ), f"Chosen loss masking spans mismatch for document {i}: {document.chosen_span} != {dataset.get_document(i).chosen_span}."

            assert np.array_equal(
                dataset_item.rejected_span, document.rejected_span, equal_nan=True
            ), f"Rejected loss masking spans mismatch for document {i}: {document.rejected_span} != {dataset.get_document(i).rejected_span}."
...
def test_split_dataset():
    dataset_config_0 = IndexedDatasetConfig[GPTSample].from_dict(DATASET_DICT_0.copy())
...
def test_split_datasets_0():
    dataset_config_0 = IndexedDatasetConfig[GPTSample].from_dict(DATASET_DICT_0.copy())
    dataset_config_1 = IndexedDatasetConfig[GPTSample].from_dict(DATASET_DICT_1.copy())
    config = GPTMemmapDatasetPreparator._split_and_blend_dataset_configs(
