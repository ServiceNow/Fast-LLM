...
from .configuration_mtp_llama import MTPLlamaConfig
...
if _TRANSFORMERS_V4:
    ...
    class LlamaRotaryEmbedding(nn.Module):
        def __init__(self, config: MTPLlamaConfig, device=None):
            super().__init__()
            # Support both transformers 4.x (rope_theta + rope_scaling) and 5.x (rope_parameters)
    ...
    class LlamaAttention(nn.Module):
        ...
        def __init__(self, config: MTPLlamaConfig, layer_idx: int):
            super().__init__()
            self.config = config
    ...
    class MTPLlamaModel(LlamaPreTrainedModel):
        """
        Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`LlamaDecoderLayer`]

        Args:
            config: MTPLlamaConfig
        """

        def __init__(self, config: MTPLlamaConfig):
            ...
            self.mtp_heads = nn.ModuleList(
                [
                    ...
                    for layer_idx in range(
                        config.num_hidden_layers, config.num_hidden_layers + config.prediction_heads - 1
                    )
                ]
            )

            self.mtp_norms = nn.ModuleList(
                [LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps) for _ in range(config.prediction_heads)]
            )
    ...
    class MTPLlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
        ...
        def __init__(self, config):
            super().__init__(config)
            self.model = MTPLlamaModel(config)
            self.vocab_size = config.vocab_size
    ...
    class LlamaForQuestionAnswering(LlamaPreTrainedModel):
        ...
        def __init__(self, config):
            super().__init__(config)
            self.transformer = MTPLlamaModel(config)
    ...
    from transformers.models.llama.modeling_llama import LlamaDecoderLayer as _LlamaDecoderLayerV5
    from transformers.models.llama.modeling_llama import LlamaForCausalLM as _LlamaForCausalLMV5
    from transformers.models.llama.modeling_llama import LlamaModel as _LlamaModelV5

    class MTPLlamaModel(_LlamaModelV5):
        config_class = MTPLlamaConfig

        def __init__(self, config):
            super().__init__(config)
            del self.norm  # replaced by per-head norms below
            self.mtp_norms = nn.ModuleList(
                [LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps) for _ in range(config.prediction_heads)]
            )
            self.mtp_heads = nn.ModuleList(
                [
                    _LlamaDecoderLayerV5(config, layer_idx)
                    for layer_idx in range(
                        config.num_hidden_layers, config.num_hidden_layers + config.prediction_heads - 1
                    )
                ]
        ...
        def forward(
            ...
            **kwargs,
            ...
            # Prediction heads (layers[-1] always runs; mtp_heads only when returning all heads)
            layers_to_run = (
                [self.layers[-1]] + list(self.mtp_heads) if return_all_prediction_heads else [self.layers[-1]]
            )
            latents = []
            for i, layer in enumerate(layers_to_run):
                ...
                latents.append(self.mtp_norms[i](head_out))

            last_hidden_state = torch.stack(latents, dim=-2) if return_all_prediction_heads else latents[0]
            if output_hidden_states:
    ...
    class MTPLlamaForCausalLM(_LlamaForCausalLMV5):
        config_class = MTPLlamaConfig
        ...
        def forward(
            ...
            **kwargs,
            ...
            outputs = self.model(
                ...
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
                return_all_prediction_heads=return_all_prediction_heads,
                **kwargs,
            )
            hidden_states = outputs.last_hidden_state
            if return_all_prediction_heads:
