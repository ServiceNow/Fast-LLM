...
_VALIDATION_METRIC_FORMATS = (
    "{phase}{sub_dataset_name} @ iteration {iteration:6.0f}/{train_iters:6.0f}"
...
def format_metrics(
    metrics: dict[str, float | int], loss_defs: list[LossDef], phase: PhaseType, sub_dataset_name: str | None = None
) -> str:
    ...
    outputs = [
        _METRIC_FORMATS[phase].format(
            phase=phase,
            sub_dataset_name="" if sub_dataset_name is None else f"/{sub_dataset_name}",
