...
class Attention[ConfigType: AttentionConfig](BlockWithBias[ConfigType]):
    ...
    def _forward(
        ...
        batch_config: GPTBatchConfig = kwargs[AttentionKwargs.batch_config]
        token_dims = (batch_config.batch_dim, batch_config.sequence_q_dim)
        query = query.unflatten(0, token_dims)
        key_value = key_value.unflatten(0, token_dims)
    ...
    def get_compute_usage(self, input_: TensorMeta, kwargs: dict[str, typing.Any], config: ResourceUsageConfig) -> int:
        ...
        if config.global_:
            batch_size, sequence_q = batch_config.batch_dim.global_size, batch_config.sequence_q_dim.global_size
            # In case of sequence-data-parallel, we need to undo the shift in k-sequence-length.
            sequence_k = kwargs[AttentionKwargs.sequence_k_dim].global_size - batch_config.sequence_q_dim.size * (
                batch_config.sequence_q_dim.parallel_dim.size - batch_config.sequence_q_dim.parallel_dim.rank - 1
            )
        else:
            batch_size, sequence_q = batch_config.batch_dim.size, batch_config.sequence_q_dim.size
    ...
    def _preprocess_for_backup_attention(self, kwargs: dict[str, typing.Any]) -> None:
        ...
        if self._config.causal:
            if batch_config.sequence_length > self._backup_attention_tensor_cache_max_sequence_length:
                # Create tensor cache.
                self._backup_attention_tensor_cache_max_sequence_length = batch_config.sequence_length

                self._backup_attention_mask = torch.ones(
                    (batch_config.sequence_length, batch_config.sequence_length),
                    dtype=torch.bool,
                    device=device,
                ).tril_()
