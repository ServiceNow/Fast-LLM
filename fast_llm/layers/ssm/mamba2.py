...
class Mamba2DiscreteTest(torch.nn.Module):
    """
    This code is adapted from https://github.com/jxiw/M1/blob/537a1ca5407a786a99dc6c721873493cf8750d5e/mamba/hybrid_mamba_layer.py
    """
    ...
    def __init__(
        ...
        dt_init_std = self.dt_rank**-0.5 * dt_scale
        if config.dt_init == "constant":
            init_fill_(dt_init_std)
        elif config.dt_init == "random":
            init_uniform_(-dt_init_std, dt_init_std)
        else:
            raise NotImplementedError
    ...
    def forward(self, hidden_states, kwargs):
        ...
        zxbcA_log = self.in_proj(hidden_states)
        z, x, B, C, A_log = torch.split(
            zxbcA_log, [self.d_inner, self.d_xb, self.d_xb, self.d_inner, self.dt_rank], dim=-1
        )
        ...
        y = _mamba_chunk_scan_combined(
            ...
            dt=A_log,
            dt_softplus=True,
            A=-torch.ones(self.num_C_head, device=A_log.device),
            B=B,
            C=C,
