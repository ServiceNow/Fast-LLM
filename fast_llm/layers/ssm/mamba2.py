...
class NemotronHMamba2(Mixer):
    ...
    def __init__(
        ...
        self.norm = MambaRMSNormGated(
            inner_dim,
            eps=1e-5,
            group_size=inner_dim.size // self._local_heads,
            lr_scale=lr_scale,
    ...
    def forward(self, input_: torch.Tensor, kwargs: dict[str, typing.Any]) -> tuple[torch.Tensor, torch.Tensor | None]:
        ...
        assert _mamba_available
        assert _causal_conv1d_available
        cu_seqlens = kwargs.get(SSMKwargs.cu_seqlens)  # kwargs[SSMKwargs.cu_seqlens]
        seq_idx = kwargs.get(SSMKwargs.seq_idx)  # kwargs[SSMKwargs.seq_idx]
