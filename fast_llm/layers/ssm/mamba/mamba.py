...
from fast_llm.engine.config_utils.initialization import LambdaInitializer, init_normal_
...
class Mamba[ConfigType: Mamba](BlockLayer[ConfigType]):
    ...
    def __init__(
        ...
        # TODO: Concatenated config
        self.in_proj = self._config.x_layer.get_layer(
            hidden_dim,
        ...
        self.conv1d_weight = self._config.convolution_layer.get_weight(
            inner_dim,
            scalar_dim,
        ...
        self.x_proj = self._config.x_projection_layer.get_layer(
            ...
            auto_weight_grad_accumulation=True,
            lr_scale=self._lr_scale,
            peft=self._block_config.peft,
        )
        ...
        self.dt_proj_weight = self._config.dt_layer.get_weight(
            inner_dim,
            dt_rank_dim,
        ...
        self.dt_proj_bias = self._config.dt_layer.get_bias(
            inner_dim,
        ...
        self.A_log = self._config.a_log_layer.get_weight(
            inner_dim,
            state_dim,
        ...
        # D "skip" parameter
        self.D = self._config.d_layer.get_weight(
            inner_dim,
    ...
    def forward(
        ...
        # not, if we wanbt to support inference, we would need to implement slow path here, see https://github.com/Zyphra/Zamba2/blob/1b182f40f2257f822cc06dd785df53d67d691a15/mamba_layer.py#L172s
        out = _mamba_inner_fn(
            in_proj,
            self.conv1d_weight,
            None,
