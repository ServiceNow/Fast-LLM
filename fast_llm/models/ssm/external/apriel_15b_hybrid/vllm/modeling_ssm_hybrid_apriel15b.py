...
class Mamba2(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        # print(f"🔥 In Mamba2 forward, layer_index: {self.layer_idx}, past_key_value={len(past_key_value) if past_key_value else None}")
        cu_seqlens = None
        assert is_fast_path_available and "cuda" in self.in_proj.weight.device.type, "Only support fast path on cuda"
        cache_position = kwargs.get("cache_position", None)
        ...
        # logger.info(f"🔥 In Mamba2 forward, past_key_value={past_key_value}")
        ssm_state, conv_state = self._get_states_from_cache(past_key_value, batch)
        if use_precomputed_states:
            # ssm_state, conv_state = self._get_states_from_cache(past_key_value, batch)
