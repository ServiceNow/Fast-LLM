...
class HuggingfaceGPTModelForCausalLM(HuggingfacePreTrainedModel):
    ...
    def _get_batch(
        ...
        batch = LanguageModelPreprocessedBatch.from_batch(
            LanguageModelBatch(
                tokens=TokenBatch(tokens=input_ids.flatten(), lengths=sequence_lengths), num_tokens=input_ids.numel()
            ),
            self._fast_llm_model.get_preprocessing_config(PhaseType.inference),
            self._fast_llm_model.distributed.device,
        )
        ...
        if past_key_values is not None:
            ...
            batch.micro_batches[0].pasts = past_key_values
