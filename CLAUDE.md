...
The project uses a venv at `venv/`. Always activate it before running any Python commands:
...
Always redirect output to a file to avoid truncation, e.g. `pytest ... 2>&1 | tee /tmp/pytest_out.txt`.
