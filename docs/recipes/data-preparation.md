...
We'll use [the-stack](https://huggingface.co/datasets/bigcode/the-stack) dataset, which is one of the largest collections of permissively-licensed source code files for this tutorial.  
...
## ⚙️ Step 2: Prepare the tokenizer and configs for conversion of data to gpt_mmap format

In this step, we download the tokenizer and create configs required to run the data preparation scripts.

We'll use Mistral-Nemo-Base-2407 tokenizer. Let's create a folder first
```bash
mkdir -p ./prep-stack-tutorial/checkpoints/Mistral-Nemo-Base-2407
```

And then download the tokenizer with the script
```python
...
Fast-LLM's prepare command processes the dataset by tokenizing and saving it in mmap format.
...
That is all! Once the jobs complete, you'll see the data in gpt_mmap format in `./prep-stack-tutorial/tokenized/Mistral-Nemo-Base-2407` which can be used with Fast-LLM to set off a training run.