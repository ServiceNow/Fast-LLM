...
-   **Self-review with Claude Code before requesting human review**: in [Claude Code](https://claude.com/claude-code), run `/review-coarse` on your branch (structure, correctness, scope, test coverage), address its findings, and push. Then run `/review-fine` (naming, comments, dead code, style) on the cleaned-up branch. Splitting the passes matters — coarse-pass fixes routinely invalidate fine-pass feedback, so doing both at once wastes effort. Address (or knowingly dismiss) findings from both passes before pinging a reviewer.
