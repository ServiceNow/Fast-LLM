...
- **Performance**: a new feature must add **zero overhead when unused** — no extra kernel launches, no slower GPU code path, no extra CPU work in hot paths (config validation, layer construction, schedule loops). Gate new behavior behind a config flag that short-circuits cheaply when off; flag any new always-on cost, even small ones, and any branch that makes the disabled path measurably slower than before.
