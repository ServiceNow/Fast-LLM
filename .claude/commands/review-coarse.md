...
<!-- Maintenance: the "How to read the diff" and "Output" sections must stay byte-identical with .claude/commands/review-fine.md. Slash commands have no include mechanism; keep them in sync by hand. -->
...
- **Necessity**: code that doesn't earn its place — new modules, classes, abstractions, code paths, validation, fallbacks, or backwards-compat shims that have no real consumer or that go beyond what the task requires; config options that don't toggle meaningful behavior.
